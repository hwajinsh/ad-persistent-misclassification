...
Fisher-Freeman-Halton exact test, with Cramer's V as the effect size.
...
def fisher_exact_rxc(contingency_table, max_tables=2_000_000):
    """Fisher-Freeman-Halton exact test for a 2 x C table.

    The subtype comparison has small expected counts (every Voted-FN cell is
    below 5), so the chi-square approximation does not hold and the exact test
    is the appropriate one. SciPy's fisher_exact falls back to Monte Carlo
    resampling for tables larger than 2 x 2, which returns a different p-value
    on every call; this enumerates the full reference set instead so the
    reported p-value is reproducible.
    """
